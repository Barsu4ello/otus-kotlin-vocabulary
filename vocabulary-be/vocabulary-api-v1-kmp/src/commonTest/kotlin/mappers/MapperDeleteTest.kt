...
class MapperDeleteTest {
    ...
    @Test
    fun fromTransport() {
        val word = VcblWordStub.get()
        val req = WordDeleteRequest(
            debug = WordDebug(
                mode = WordRequestDebugMode.STUB,
                stub = WordRequestDebugStubs.SUCCESS,
            ),
            word = VcblWordStub.get().toTransportDeleteWord()
        )
        ...
        assertEquals(word.id.toTransportWord(), context.wordRequest.id.asString())
        assertEquals(word.lock.toTransportWord(), context.wordRequest.lock.asString())
    }
    ...
    fun toTransport() {
        ...
        val req = context.toTransportWord() as WordDeleteResponse
